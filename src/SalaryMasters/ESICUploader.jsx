...
export default function ESICUploader({ isVisible, setIsVisible, refreshData }) {
  ...
  const handleUpload = async () => {
    ...
    formData.append('excelFile', fileList[0].originFileObj)

    try {
      const res = await axiosInstance.post('/api/Uploader/UploadEmployeeESIC', formData, {
        headers: { Accept: '*/*' },
      })
  ...
  return (
    ...
    >
      <Row gutter={[24, 16]}>
        {/* Left column */}
        <Col xs={24} md={12}>
          <a href="/ESICUploader.xlsx" download style={{ display: 'inline-block', width: '100%' }}>
            <Button
