...
export default function Uploader({ isVisible, setIsVisible, refreshData }) {
  ...
  const onChange = ({ fileList: newList }) => setFileList(newList.slice(-1)) // only last 1 file
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await axiosInstance.post('/api/GroupWiseStoreCodeMapping/upload', formData, {
        headers: { Accept: '*/*' },
      })
  ...
  return (
    ...
    >
      <Row gutter={[24, 16]}>
        ...
        <Col xs={24} md={12}>
          <a
            href="/GroupStoreCodeMapping.xlsx"
            download
