...
export default function PolicyUploader({ isVisible, setIsVisible, refreshData }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await axiosInstance.post('/api/HolidayMaster/ImportPolicyDesignation', formData, {
        headers: { Accept: '*/*' },
      })
  ...
  return (
    ...
    >
      <Row gutter={[24, 16]}>
        {/* Left column */}
        <Col xs={24} md={12}>
          <a href="/Upload Policy.xlsx" download style={{ display: 'inline-block', width: '100%' }}>
            <Button
          ...
          <Paragraph>Delete sample data before uploading.</Paragraph>
