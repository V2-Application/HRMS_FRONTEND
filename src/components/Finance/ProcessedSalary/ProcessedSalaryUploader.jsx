...
export default function ProcessedSalaryUploader({ isVisible, setIsVisible, refreshData }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await axiosInstance.post(
        '/api/EmpAttendanceViewSnapshot/process-excel-upload',
        formData,
        {
          headers: { Accept: '*/*' },
        },
  ...
  return (
    ...
    >
      <Row gutter={[24, 16]}>
        ...
        <Col xs={24} md={12}>
          <a
            href="/FinanceProcessSalary.xlsx"
            download
