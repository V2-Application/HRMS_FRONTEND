...
import ReturnByBankColumns from './ReturnByBankColumns'
...
const ReturnByBank = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetchSalaryStatusList(5, monthVal.format('MMM-YY'))
      ...
      console.error('Return by bank get error:', error)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const { columns, totalWidth } = ReturnByBankColumns({
