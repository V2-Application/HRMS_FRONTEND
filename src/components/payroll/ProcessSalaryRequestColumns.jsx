...
const ProcessSalaryRequestColumns = ({
  onUpdateStatus,
  isUpdatingId,
  fetchSalData,
  isInfoLoading,
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: (a, b) => {
        const da = a.runAt ? new Date(a.runAt).getTime() : 0
        const db = b.runAt ? new Date(b.runAt).getTime() : 0
        return da - db
      },
      sortDirections: ['ascend', 'descend'],
      defaultSortOrder: 'descend', // optional – remove if you don't want default sorting
    },
    {
      title: 'Loc Code',
    ...
    // 🔥 ACTIONS COLUMN (UPDATED: 1 = Approve, -1 = Reject)
    {
      title: 'Actions',
      dataIndex: 'actions',
      key: 'actions',
      ...
      render: (_, record) => {
        ...
        return (
          <Space>
            <Tooltip title="Process (Approve)" placement="top">
              <Popconfirm
                title="approve"
                description="Are you sure you want to process this operation?"
                okText="Yes"
                cancelText="No"
                // ✅ 1 = Approve
                onConfirm={() => onUpdateStatus(record?.id, 1)}
                placement="left"
              >
                <Button loading={loading} icon={<CheckOutlined />} />
              </Popconfirm>
            </Tooltip>

            <Tooltip placement="top" title="Return by Bank (Reject)">
              <Popconfirm
                title="Cancel"
                ...
                placement="left"
              >
                <Button loading={loading} icon={<CloseOutlined />} />
              </Popconfirm>
            </Tooltip>
          </Space>
        )
...
export default ProcessSalaryRequestColumns
