...
export default function WeeklyOffHolidayUploader({ isVisible, setIsVisible, refreshData }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await axiosInstance.post(
        '/api/LocationDesignationWeeklyOffHolidayMaster/upload',
        formData,
        {
          headers: { Accept: '*/*' },
        },
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
        <div style={{ flex: 1 }}>
          <a href="/LocationDesgWeeklyOffHolidays.xlsx" download>
            <Button icon={<DownloadOutlined />} type="primary">
              Download Sample Sheet
            </Button>
          </a>

          <Paragraph type="secondary" style={{ fontSize: 14, marginTop: 8, fontWeight: 600 }}>
            * Download, fill out, then upload the sample Excel file. Expected format: MMM-YY (e.g.,
            May-25, Jun-25)
          </Paragraph>
