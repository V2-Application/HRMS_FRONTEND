...
export default function EmpTDSUploader({ isVisible, setIsVisible, refreshData }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await axiosInstance.post('/api/Uploader/UploadEmpTDSTable', formData, {
        headers: { Accept: '*/*' },
      })
        ...
        <Col xs={24} md={12}>
          ...
          <Paragraph type="secondary" style={{ fontSize: 14, marginTop: 8 }}>
            * Download, fill out, then upload the sample Excel file. Month format examples: for June
            – <Text code>June-25</Text>, for August – <Text code>Aug-25</Text>.
          </Paragraph>
