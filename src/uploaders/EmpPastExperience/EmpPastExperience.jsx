...
import EmpPastExperienceUploader from './EmpPastExperienceUploader'
...
const EmpPastExperience = () => {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
        ...
        <div>
          ...
          <div style={{ fontWeight: 600, fontSize: 10, color: '#1890ff', textAlign: 'center' }}>
            {record.froM_DATE ? record.froM_DATE.split('T')[0] : '-'}
          </div>
        </div>
        <div>
          <div
          ...
          <div style={{ fontWeight: 600, fontSize: 10, color: '#ff4d4f', textAlign: 'center' }}>
            {record.tO_DATE ? record.tO_DATE.split('T')[0] : '-'}
          </div>
        </div>
      </div>
    </div>
  ...
  // Mobile columns - MAIN ROW: Code, Company, Designation, CTC + Expand (5 columns)
  const mobileColumns = [
    {
      title: 'Code',
      dataIndex: 'e_CODE',
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetchEmpPastExperience()
  ...
  return (
    <>
      <Pageheading title="Emp Past Experience Uploader" />
      <ToastContainer
        position="top-right"
        autoClose={2000}
...
const TableBulkActionIcons = ({
  ...
  const downloadDataInExcel = () => {
    const columns = [
      ...
      { header: 'Comp S. No.', key: 'comP_S_NO' },
      { header: 'Comp Name', key: 'comP_NAME' },
      { header: 'Location', key: 'location' },
      { header: 'Designation', key: 'designation' },
      { header: 'From Date', key: 'froM_DATE' },
      { header: 'To Date', key: 'tO_DATE' },
      { header: 'Last CTC', key: 'lasT_CTC' },
    ]

    setlodingLocal(true)

    const response = exportExcelFromFrontend(columns, filteredData, 'EmpPastExperience.xlsx')
        ...
        <Row>
          <Col>
            {actionsMap?.upload?.actionStatus && (
              <Tooltip placement="top" title={'Upload Emp Past Experience'}>
...
export default EmpPastExperience
