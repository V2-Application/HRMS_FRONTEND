...
import LeaveOpeningBalUploader from './LeaveOpeningBalUploader'
...
const LeaveOpeningBalMaster = () => {
  ...
  const mobileColumns = [
    ...
    },
    {
      ...
      width: 40,
      render: (val) => (
        <div style={{ fontSize: 10, fontWeight: 600, color: '#1890ff', textAlign: 'center' }}>
          {val || 0}
        </div>
      ),
    },
    ...
    },
    {
      ...
      width: 40,
      render: (val) => (
        <div style={{ fontSize: 10, fontWeight: 600, color: '#fa8c16', textAlign: 'center' }}>
          {val || 0}
        </div>
      ),
    },
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetchLeaveOpeningBal()
  ...
  return (
    <>
      <Pageheading title="Leave Opening Bal Uploader" />
      <ToastContainer
        position="top-right"
        autoClose={2000}
...
const TableBulkActionIcons = ({
  ...
  const downloadDataInExcel = () => {
    ...
    const response = exportExcelFromFrontend(columns, filteredData, 'LeaveOpeningBalMaster.xlsx')
        ...
        <Row>
          <Col>
            {actionsMap?.upload?.actionStatus && (
              <Tooltip placement="top" title={'Upload Leave Opening Bal'}>
...
export default LeaveOpeningBalMaster
