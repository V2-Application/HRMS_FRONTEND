...
import EmpPersonalDetailsUploader from './EmpPersonalDetailsUploader'
...
const EmpPersonalDetailsMaster = () => {
  ...
  // Mobile expanded row - ALL remaining fields in ONE row (4 columns)
  // Mobile expanded row - 3 remaining fields in ONE row
    ...
    <div style={{ padding: 12, background: '#fafafa', fontSize: 11 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
        ...
        <div>
          ...
          >
            {record.d_O_B ? record.d_O_B.split('T')[0] : '-'}
          </div>
        </div>
      </div>
    </div>
  ...
  // Mobile columns - MAIN ROW: Code, Name, Mobile, Gender + Expand (5 columns)
  const mobileColumns = [
    {
      title: 'Code',
      dataIndex: 'e_CODE',
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetchEmpPersonalDetails()
  ...
  return (
    <>
      <Pageheading title="Emp Personal Details Uploader" />
      <ToastContainer
        position="top-right"
        autoClose={2000}
...
const TableBulkActionIcons = ({
  ...
  const downloadDataInExcel = () => {
    const columns = [
      ...
      { header: 'Address Details', key: 'addresS_DETAIL' },
      { header: 'D.O.B.', key: 'd_O_B' },
    ]

    setlodingLocal(true)

    const response = exportExcelFromFrontend(columns, filteredData, 'EmpPersonalDetailsMaster.xlsx')
        ...
        <Row>
          <Col>
            {actionsMap?.upload?.actionStatus && (
              <Tooltip placement="top" title={'Upload Emp Personal Details'}>
...
export default EmpPersonalDetailsMaster
