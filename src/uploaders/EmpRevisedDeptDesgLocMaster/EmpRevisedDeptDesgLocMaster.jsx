...
import {
  ...
  fetchEmpRevisedDeptDesgLocDetails,
...
import EmpRevisedDeptDesgLocUploader from './EmpRevisedDeptDesgLocUploader'
...
const EmpRevisedDeptDesgLocMaster = () => {
  ...
  // Mobile columns - ALL 5 fields in single row (no expansion)
  const mobileColumns = [
    {
      title: 'Code',
      dataIndex: 'e_CODE',
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetchEmpRevisedDeptDesgLocDetails()
  ...
  return (
    <>
      <Pageheading title="Emp Revised Dept-Desg-Loc Uploader" />
      <ToastContainer
        position="top-right"
        autoClose={2000}
...
const TableBulkActionIcons = ({
  ...
  const downloadDataInExcel = () => {
    const columns = [
      ...
      { header: 'New Department', key: 'neW_DEPARTMENT' },
      { header: 'New Designation', key: 'neW_DESIGNATION' },
      { header: 'Posted Location', key: 'posteD_LOCATION' },
      { header: 'Posted Date', key: 'posteD_DT' },
    ]
    ...
    const response = exportExcelFromFrontend(
      columns,
      filteredData,
      'EmpRevisedDeptDesgLocMaster.xlsx',
    )
        ...
        <Row>
          <Col>
            {actionsMap?.upload?.actionStatus && (
              <Tooltip placement="top" title={'Upload Employee Revised Dept-Desg-Loc'}>
...
export default EmpRevisedDeptDesgLocMaster
