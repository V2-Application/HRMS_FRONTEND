...
export default function PayrollUploadModal({ isVisible, setIsVisible, refreshData }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await axiosInstance.post('/api/BankTransfer/upload-bank-transfer', formData)
      ...
      const errMsg = err?.response?.data || err?.response?.data?.message || 'Error uploading file.'
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
        <div style={{ flex: 1 }}>
          <a href="/GivenToBankSampleSheet.xlsx" download>
            <Button icon={<DownloadOutlined />} type="primary">
              Download “Given to Bank” Sample Template
            </Button>
          </a>
