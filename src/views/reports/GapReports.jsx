...
const ALL_REPORTS = [
  { key: 'location', name: 'Location-wise Absconding Report', category: 'Absconding' },
  { key: 'employee', name: 'Employee-wise Absconding Report', category: 'Absconding' },
  { key: 'mispunch', name: 'TD/MTD Mis-Punch Gap Report (Employee-wise)', category: 'Attendance' },
  { key: 'mispunch-loc', name: 'TD/MTD Mis-Punch Gap Report (Location-wise)', category: 'Attendance' },
  { key: 'geofence-loc', name: 'TD/MTD Geo-Fencing Gap Report (Location-wise)', category: 'Attendance' },
  { key: 'geofence-emp', name: 'TD/MTD Geo-Fencing Gap Report (Employee-wise)', category: 'Attendance' },
  { key: 'regularization-loc', name: 'TD/MTD Regularization Gap Report (Location-wise)', category: 'Attendance' },
  { key: 'regularization-emp', name: 'TD/MTD Regularization Gap Report (Employee-wise)', category: 'Attendance' },
  { key: 'lastpunch-sep', name: 'Last Punch vs Separation High Ageing Gap Report', category: 'Separation' },
  { key: 'lastpunch-after-sep', name: 'Last Punching Shows After Separation Gap Report', category: 'Separation' },
  { key: 'sep-fnf-pending', name: 'Separated But F&F Pending Gap Report', category: 'Separation' },
  { key: 'sep-lastpunch-missing', name: 'Separated But Last Punch Date Missing Gap Report', category: 'Separation' },
  { key: 'sep-resignation-missing', name: 'Separated But Resignation Missing Gap Report', category: 'Separation' },
  { key: 'rm-geofence-pending', name: 'TD/MTD RM Geo-Fencing Approval Pending Gap Report', category: 'Approval Pending' },
  { key: 'rm-regularization-pending', name: 'TD/MTD RM Regularization Approval Pending Gap Report', category: 'Approval Pending' },
  { key: 'audit-regularization-pending', name: 'TD/MTD Audit Regularization Approval Pending Gap Report', category: 'Approval Pending' },
  { key: 'absent-loc', name: 'Location-wise Absent TD/MTD Gap Report', category: 'Attendance' },
  { key: 'actemp-vs-attend-loc', name: 'Location-wise Act Emp vs Act Attendance Gap Report', category: 'Manpower / Budget' },
  { key: 'bgtemp-vs-attend-loc', name: 'Location-wise Bgt Emp vs Act Emp Gap Report', category: 'Manpower / Budget' },
  { key: 'bgtemp-vs-actattend-loc', name: 'Location-wise Bgt Emp vs Act Attendance Gap Report', category: 'Manpower / Budget' },
...
const GapReports = () => {
  ...
  // Distinct categories present in the current report list (for the Category dropdown).
  const categories = [
    ALL_CATEGORIES,
    ...Array.from(new Set(reports.map((r) => r.category).filter(Boolean))),
  ]

  // Reports shown in the Report dropdown, filtered by the chosen category.
  const filteredReports =
    selectedCategory === ALL_CATEGORIES
      ? reports
      : reports.filter((r) => r.category === selectedCategory)

  // Switching category: if the currently-selected report isn't in the new list, jump to the first one.
  const handleCategoryChange = (cat) => {
    setSelectedCategory(cat)
    const next = cat === ALL_CATEGORIES ? reports : reports.filter((r) => r.category === cat)
    if (next.length && !next.some((r) => r.key === selected)) {
      ...
      try {
        ...
        apiList.forEach((r) => {
          if (r && r.key) {
            ...
            // Merge API entry on top, but keep the local category if the backend didn't send one.
            byKey.set(r.key, { ...existing, ...r, category: r.category || existing.category })
          }
        })
            ...
            <Space wrap size="middle" align="end">
              ...
              <div>
                ...
                <Select
                  ...
                  options={filteredReports.map((r) => ({ value: r.key, label: r.name, title: r.name }))}
                />
              </div>
              <div>
