...
export default function LWFMasterUploader({ isVisible, setIsVisible, refreshData }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await axiosInstance.post('/api/LWFMaster/upload', formData, {
        headers: { Accept: '*/*' },
      })
      console.log('upload lwf master res:', res)
  ...
  return (
    ...
    >
      <Row gutter={[24, 16]}>
        {/* Left column */}
        <Col xs={24} md={12}>
          <a href="/LWFMaster.xlsx" download style={{ display: 'inline-block', width: '100%' }}>
            <Button
