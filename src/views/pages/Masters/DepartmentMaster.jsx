...
const DepartmentMaster = () => {
  ...
  const fetchRows = useCallback(async () => {
    setLoading(true)
    try {
      const res = await getAllDepartments({ onlyInactive, searchTerm })
  ...
  const handleSubmit = async () => {
    try {
      ...
      setSubmitting(true)
      const payload = {
        DepartmentId: editing?.departmentId ?? editing?.DepartmentId ?? null,
      ...
      if (ok) {
        message.success(res?.data?.Message || (editing ? 'Department updated' : 'Department created'))
        closeModal()
        fetchRows()
      } else {
      ...
      if (err?.errorFields) return // form validation errors handled inline
  ...
  const handleToggle = async (row, nextActive) => {
    try {
      const res = await toggleDepartmentActive({
    ...
    () => [
      {
        ...
        key: 'departmentId',
        width: 90,
        render: (v, r) => v ?? r.DepartmentId,
        sorter: (a, b) =>
          (a.departmentId ?? a.DepartmentId ?? 0) - (b.departmentId ?? b.DepartmentId ?? 0),
      },
      {
        title: 'Department Name',
        dataIndex: 'departmentName',
        key: 'departmentName',
        render: (v, r) => v ?? r.DepartmentName,
        sorter: (a, b) =>
          (a.departmentName ?? a.DepartmentName ?? '').localeCompare(
        ...
        render: (_, row) => {
          ...
          return (
            <Space>
              ...
              <Popconfirm
                title={active ? 'Deactivate this department?' : 'Activate this department?'}
  ...
  const downloadTemplate = () => {
    ...
    saveAs(new Blob([out], { type: 'application/octet-stream' }), 'Departments_Template.xlsx')
  }
  ...
  const handleUpload = async () => {
    ...
    try {
      setUploading(true)
      const res = await uploadDepartmentsExcel(uploadFile)
      const data = res?.data
      setUploadResult({
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <Table
        rowKey={(r) => r.departmentId ?? r.DepartmentId}
        loading={loading}
        columns={columns}
        dataSource={rows}
        pagination={{
      ...
      >
        <Form form={form} layout="vertical" preserve={false}>
          <Form.Item
            ...
            name="departmentName"
            rules={[{ required: true, message: 'Department name is required' }, { max: 200 }]}
          >
      ...
      >
        <Row gutter={[24, 16]}>
          ...
          <Col xs={24} md={12}>
            ...
            <Paragraph style={{ marginBottom: 0, marginTop: 6 }}>1. DEPARTMENT NAME</Paragraph>
          </Col>
        </Row>
